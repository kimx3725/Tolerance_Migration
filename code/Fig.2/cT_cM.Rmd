...
pacman::p_load(deSolve,reshape2,akima,lattice,ggplot2)
```
...
```{r}
#df.cTcM <- as.data.frame(lapply(cTcM,unlist))
#df.cTcM <- subset(df.cTcM,select=-c(population))
#df.cTcM %>% group_by(cT) %>% filter(!any())
#library(dplyr)
#cTcM3 %>% filter(X1 < 1)
```

```{r}
#df_cTcM <- as.data.frame(cTcM2)
#filter(df_cTcM,alpha=="0.9")
#which(df_cTcM$cT=="0.9")
#colnames(df_cTcM)
#cTcM2$X1[which(cTcM2$cT=="0.9")] <- NA
#cTcM2$Y1[which(cTcM2$cT=="0.9")] <- NA
#cTcM2$X2[which(cTcM2$cT=="0.9")] <- NA
#cTcM2$Y2[which(cTcM2$cT=="0.9")] <- NA
#cTcM2$X1[which(cTcM2$cT=="1")] <- NA
#cTcM2$Y1[which(cTcM2$cT=="1")] <- NA
#cTcM2$X2[which(cTcM2$cT=="1")] <- NA
#cTcM2$Y2[which(cTcM2$cT=="1")] <- NA
```
...
```{r}
# Plot
plot(cT~cM,data=cTcM3,type="n")
points(M~cM,data=cTcM3,type="p",col="red")
points(R~cM,data=cTcM3,type="p",col="blue")
```
...
#initial model
M.plot <- ggplot(cTcM3,aes(x,y)) + geom_tile(aes(fill=Mz),interpolate = TRUE) + xlab("cM") + ylab("cT")

# modified model
M.plot1 <- M.plot + labs(fill= "Population") + theme(legend.title = element_text(size = 9),legend.text = element_text(size=6)) + ggtitle("a, Fraction of Migrants") + scale_fill_gradientn(colors=pal,limits=c(0,1),breaks=b,labels=format(b)) + scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0))
...
#initial model
R.plot <- ggplot(cTcM3,aes(x,y)) + geom_tile(aes(fill=Rz),interpolate = TRUE) + xlab("cM") + ylab("cT")

# modified model
R.plot1 <- R.plot + labs(fill= "Population") + theme(legend.title = element_text(size = 9),legend.text = element_text(size=6)) + ggtitle("b, Fraction of Residents") + scale_fill_gradientn(colors=pal,limits=c(0,1),breaks=b,labels=format(b))+ scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0))
